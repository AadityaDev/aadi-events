<template>
  <section class="dashboard-user-edit-page h-min-screen bg-white p-5 mt-12">
    ...
    <form>
      ...
      >
        ...
        <span v-if="statusFilter" class="text-gray-600">
          <label class="border border-gray-400 mb-2 bg-gray-300 inline-block px-2 mr-2 pointer rounded">
            Status: {{ parseInt(this.statusFilter) === 1 ?'In-Active':'Active' }}
          </label>
        </span>
      </div>
      ...
      <section v-if="filteredResults">
        <div v-for="(companyVerticalRow, index) in filteredResults" :key="index" class="userCard border border-gray-400 my-6 p-1 p-2 rounded-lg">
          <!-- User comapnies list -->
        ...
        <div v-if="userRole && (userRole === 'voc' || userRole === 'vod') && !enableActivateButton" class="cta-cont text-right mt-12 mb-12">
          <button
        ...
        <div v-if="modalDisableUser" class="modal fixed w-full h-full top-0 left-0 flex items-center justify-center z-10">
          ...
          <div class="modal-container w-5/6 bg-white mx-4 rounded shadow-lg z-10 overflow-y-auto">
            ...
            <div class="flex justify-around py-2 rounded-lg">
              <button class="border-2 border-red-500 font-bold px-3 rounded text-red-500 text-sm" @click="DeleteUser">
                Delete
              </button>
      ...
      <transition name="bounce">
        <div v-if="modalActivateUser" class="modal fixed w-full h-full top-0 left-0 flex items-center justify-center z-10">
          ...
          <div class="modal-container w-5/6 bg-white mx-4 rounded shadow-lg z-10 overflow-y-auto">
            ...
            <div class="flex justify-around py-2 rounded-lg">
              <button class="border-2 border-green-500 font-bold px-3 rounded text-green-500 text-sm" @click="ActivateUser">
                Enable
              </button>
              <button class="border-2 border-gray-500 font-bold px-3 rounded text-gray-500 text-sm" @click="leaveConfirmBox">
                Cancel
      ...
      <transition name="bounce">
        <div v-if="popupUserActionDisable" class="modal fixed w-full h-full top-0 left-0 flex items-center justify-center z-10">
          ...
          <div class="modal-container w-5/6 bg-white mx-4 rounded shadow-lg z-10 overflow-y-auto">
            ...
            <h2 class="font-bold text-black px-2 pt-6 pb-0 text-center text-lg">
              {{ popupRole }},
            </h2>

            <h3 class="font-bold text-black px-2 pb-6 pt-0 text-center text-sm">
              {{ popupText }}
            </h3>
            <div class="flex justify-around py-2 rounded-lg">
              <button class="border-2 border-gray-600 font-bold px-3 rounded text-gray-600 text-sm" @click="leave">
                Cancel
              </button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    async softDeleteUserCompany(index) {
      ...
      if (this.currentRoleUser === 'voc') {
        //  console.log("call voc");
        console.log(parseInt(data.fetchCompanyVocAndVodFromCompVertical[0].voc, 10));
